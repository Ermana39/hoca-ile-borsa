...
function ortalamaFaizHesapla() {
  const faizler = bankaFaizListesi.map((item) => faizMetniniSayiyaCevir(item.maxFaiz));
  const toplam = faizler.reduce((acc, item) => acc + item, 0);
  return faizler.length > 0 ? Number((toplam / faizler.length).toFixed(2)) : 0;
}

export default function MevduatFaiziOranlariPage() {
  addView("/mevduat-kredi-faizleri/mevduat-faizi-oranlari");

  const ortalamaFaiz = ortalamaFaizHesapla();
  const enYuksekFaiz = bankaFaizListesi[0]?.maxFaiz ?? "-";
  const bankaSayisi = bankaFaizListesi.length;
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-zinc-200 bg-white p-5">
            <div className="text-sm text-zinc-500">Banka Sayısı</div>
            <div className="mt-2 text-2xl font-bold text-zinc-900">{bankaSayisi}</div>
          </div>
          <div className="rounded-2xl border border-zinc-200 bg-white p-5">
            <div className="text-sm text-zinc-500">Ortalama Faiz</div>
            <div className="mt-2 text-2xl font-bold text-zinc-900">%{ortalamaFaiz}</div>
          </div>
          <div className="rounded-2xl border border-zinc-200 bg-white p-5">
            <div className="text-sm text-zinc-500">En Yüksek Faiz</div>
            <div className="mt-2 text-2xl font-bold text-zinc-900">{enYuksekFaiz}</div>
          </div>
        </section>
