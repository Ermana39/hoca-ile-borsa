...
function hisseleriOku() {
  const rows = (dususTrendiData.rows || []) as JsonRow[];

  if (!rows.length) return [];

  const columns =
    Array.isArray(dususTrendiData.columns) && dususTrendiData.columns.length > 0
      ? dususTrendiData.columns
      : Object.keys(rows[0] || {});
    ...
    <main className="min-h-screen bg-white px-4 py-6 md:px-6">
      <div className="mx-auto max-w-6xl">
        ...
        <p className="mb-3 max-w-3xl text-base text-zinc-600">
          5, 13, 21, 55, 89, 144, 233 hareketli ortalama altında olan hisseler
        </p>
