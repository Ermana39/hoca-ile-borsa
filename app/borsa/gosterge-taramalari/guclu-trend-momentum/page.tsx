...
function hisseleriOku() {
  const rows = (gucluTrendMomentumData.rows || []) as JsonRow[];

  if (!rows.length) return [];

  const columns =
    Array.isArray(gucluTrendMomentumData.columns) &&
    gucluTrendMomentumData.columns.length > 0
      ? gucluTrendMomentumData.columns
      : Object.keys(rows[0] || {});
    ...
    <main className="min-h-screen bg-white px-4 py-6 md:px-6">
      <div className="mx-auto max-w-6xl">
        ...
        <p className="mb-3 max-w-3xl text-base text-zinc-600">
          Hareketli ortalama dizilimi, StochRSI ve MACD göstergelerine göre
          güçlü yükseliş trendi ve pozitif momentum gösteren hisseler
        </p>
